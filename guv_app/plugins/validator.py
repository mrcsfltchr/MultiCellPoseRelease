...
def validate_visualization_mask(
    ...
    3. ID consistency: viz_mask must only contain IDs present in original_mask (or 0),
       unless allow_new_labels=True for plugin-specific review IDs such as track IDs.
    ...
    Args:
        ...
        allow_new_labels: Permit visualization IDs larger than the original mask IDs.
    ...
    # Ensure all IDs in viz_mask exist in original_mask unless a plugin is using
    # its visualization mask as an editable review layer with plugin-specific IDs.
    if not allow_new_labels and original_mask.size > 0 and viz_mask.size > 0:
