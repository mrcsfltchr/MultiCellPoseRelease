...
def test_save_prediction_shared_mode_uses_result_masks_for_non_current_timeseries_frame(tmp_path):
    ...
    assert saved["channel_mask_mode"] == "shared"
    assert saved["channel_segmentations"] is None


def test_save_prediction_preserves_existing_per_channel_file_for_non_current_timeseries_frame(tmp_path):
    movie_path = tmp_path / "movie.tif"
    ...
    model.filename = str(movie_path)
    model.frame_id = "T0"
    model.raw_image = np.zeros((12, 12, 3), dtype=np.float32)
    model.image_is_rgb = False
    model.channel_mask_mode = "per_channel"
    model.NZ = 1
    model.Ly = 12
    model.Lx = 12
    ...
    np.save(
        saved_path,
        {
            "masks": live_masks.squeeze(),
            "channel_mask_mode": "per_channel",
    ...
    np.testing.assert_array_equal(saved["masks"], result_masks)
    assert saved["channel_mask_mode"] == "per_channel"
