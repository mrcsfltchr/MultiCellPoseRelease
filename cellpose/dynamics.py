...
def labels_to_flows(labels, files=None, device=None, redo_flows=False, niter=None,
    ...
    # compatibility: if labels are provided as (1, C, Ly, Lx) where 1 is a singleton Z-dimension
    # then squeeze the Z-axis so shape is (C, Ly, Lx) and treat as precomputed flows
    for n in range(nimg):
        if labels[n].ndim == 4 and labels[n].shape[0] == 1 and labels[n].shape[1] > 1:
            labels[n] = labels[n].squeeze(0)
    ...
    if labels[0].shape[0] == 1 or labels[0].ndim < 3 or redo_flows:
        ...
        for n in iterator(nimg):
            ...
            # print(f"labels shape: {labels[n].shape}")
            # print(f"these are in labels: {np.unique(labels[n][0])}")
            ...
            if files is not None:
                ...
                # print(f">>> saving flows for {file_name}")
