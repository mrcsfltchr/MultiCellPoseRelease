...
def _forward(net, x):
    ...
    # print(f">> about to evaluate x on device: {X.device}")
    net.eval()
    # print(f">> net on device: {net.device}")
    with torch.no_grad():
        y, style = net(X)[:2]
    del X
    y = _from_device(y)
...
def run_net(net, imgi, batch_size=8, augment=False, tile_overlap=0.1, bsize=224,
    ...
    # print(f"Lz:{Lz} Ly0:{Ly0} Lx0: {Lx0} nchan:{nchan}")
    if rsz is not None:
    ...
    ntiles = ny * nx
    # print(f"number of tiles: {ntiles}")
