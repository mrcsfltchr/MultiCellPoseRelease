...
def convert_image(x, channel_axis=None, z_axis=None, do_3D=False):
    ...
    # allow a batch of 2D images with an explicit channel axis
    if ndim == 4 and not do_3D:
        ...
        if channel_axis is None:
            raise ValueError(
                "4D input provided with do_3D=False but channel_axis is None. "
                "Specify channel_axis (e.g. -1 for (N, Y, X, C)) or pass a list of images."
            )
        if channel_axis == 0:
            raise ValueError("channel_axis cannot be the batch axis (0) for 4D batched 2D input.")

        # Interpret as batch dimension + 2D image with channels.
        x = np.moveaxis(x, channel_axis, -1)
        n_channels = x.shape[-1]
        if n_channels != 3:
            n_chans_to_copy = min(3, n_channels)
            if n_channels > 3:
                transforms_logger.warning("more than 3 channels provided, only segmenting on first 3 channels")
                x = x[..., :n_chans_to_copy]
            else:
                pad_width = [(0, 0), (0, 0), (0, 0), (0, 3 - n_chans_to_copy)]
                x = np.pad(x, pad_width, mode="constant", constant_values=0)
        return x
...
def get_pad_yx(Ly, Lx, div=16, extra=1, min_size=None):
    # compute required padding to hit both divisibility and min_size targets
    if min_size is None or Ly >= min_size[-2]:
        Lpad_y = int(div * np.ceil(Ly / div) - Ly)
    else:
        Lpad_y = min_size[-2] - Ly
    ...
    if min_size is None or Lx >= min_size[-1]:
        Lpad_x = int(div * np.ceil(Lx / div) - Lx)
    else:
        Lpad_x = min_size[-1] - Lx

    if Lpad_x == 0:
        xpad1 = xpad2 = 0
    else:
        xpad1 = extra * div // 2 + Lpad_x // 2
        xpad2 = extra * div // 2 + Lpad_x - Lpad_x // 2
