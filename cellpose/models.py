...
class CellposeModel():
    ...
    def __init__(self, gpu=False, pretrained_model="cpsam", model_type=None,
        ...
        print(f">> assigned model device: {self.device}")
        ...
        try:
            if semantic_classes is not None and os.path.basename(self.pretrained_model) == "cpsam":
                nclasses = max(int(semantic_classes), 4)
                if nclasses > 0:
                    if not os.path.exists(self.pretrained_model):
                        cache_CPSAM_model_path()
                    net = Transformer(rdrop=0.4).to(self.device)
                    self.net = net
                    self.net.to(self.device)
                    models_logger.info(f">>>> loading model {self.pretrained_model}")
                    try:
                        self.net.load_model(self.pretrained_model, device=self.device, strict=False)
                    except TypeError:
                    ...
                    net.out = nn.Conv2d(256, (nout + nclasses) * ps**2, kernel_size=1).to(self.device)
                    # background init
                    i = 0
        ...
        try:
            if os.path.exists(self.pretrained_model):
                sd = torch.load(self.pretrained_model, map_location=self.device, weights_only=True)
                # handle DataParallel keys
                if isinstance(sd, dict):
                    if 'out.weight' in sd:
                        cout = sd['out.weight'].shape[0]
                        desired_maps = max(3, cout // (ps**2))
                    elif 'module.out.weight' in sd:
                        cout = sd['module.out.weight'].shape[0]
                        desired_maps = max(3, cout // (ps**2))
        except Exception:
            pass
        try:
            if semantic_classes is not None:
                ncls = int(semantic_classes)
                if ncls > 0:
                    if os.path.basename(self.pretrained_model) == "cpsam":
                        desired_maps = max(desired_maps, ncls + 3)
                        cout = desired_maps * (ps**2)
        except Exception:
            pass
        if cout is not None:
            try:
                print(f">> No. of network output channels: {cout//(ps**2)}")
            except Exception:
                pass
        net = Transformer(nout=desired_maps).to(self.device)
        if os.path.exists(self.pretrained_model):
            self.net = net
            self.net.to(self.device)
            models_logger.info(f">>>> loading model {self.pretrained_model}")
            # be tolerant to head-size variations
            try:
                self.net.load_model(self.pretrained_model, device=self.device, strict=False)
            except TypeError:
    ...
    def eval(self, x, batch_size=8, resample=True, channels=None, channel_axis=None,
        ...
        # print(f"x shape before _run_net {x.shape}")
    ...
    def _resize_gradients(self, grads: np.ndarray, to_y_size: int, to_x_size: int, to_z_size: int = None) -> np.ndarray:
        ...
        if grads.ndim == 3:
            ...
            if to_z_size is not None:
                ...
                grads = grads.transpose(3, 0, 1, 2) # restore axes without z resize
        else:
    ...
    def _run_net(self, x, 
        ...
        if do_3D:
            ...
            if yf.shape[-1] > 3:
                # print(f">> output of run_net: yf shape = {yf.shape[-1]}")
                styles = yf[..., :-3]
...
class LoRALayer(nn.Module):
    ...
    def merge(self):
        if self.linear.weight.requires_grad:
            return
        # Merge LoRA weights into linear weights: W_new = W_old + (A @ B).T * scaling
        delta_w = (self.lora_A @ self.lora_B).T * self.scaling
        if (
            delta_w.device != self.linear.weight.device
            or delta_w.dtype != self.linear.weight.dtype
        ):
            delta_w = delta_w.to(
...
def _get_submodule_fallback(root, dotted_name):
    if dotted_name == "":
        return root
    cur = root
...
def convert_to_lora(net, rank=4, n_last_blocks=None):
    """
    Convert selected Linear layers in the encoder to LoRA layers.

    Args:
        net: model network (expects `encoder.blocks` for block-scoped conversion).
        rank: LoRA rank.
        n_last_blocks: if set, inject LoRA only in the last n encoder blocks.
            If None, inject in all encoder blocks.
    Returns:
        dict summary with conversion details.
    """
    ...
    if hasattr(net, "encoder") and hasattr(net.encoder, "blocks"):
        ...
        if n_last_blocks is None:
            applied_blocks = total_blocks
            target_prefixes = {f"encoder.blocks.{i}" for i in range(total_blocks)}
        else:
            try:
                applied_blocks = max(0, min(int(n_last_blocks), total_blocks))
            except Exception:
                applied_blocks = total_blocks
            start = max(0, total_blocks - applied_blocks)
            target_prefixes = {f"encoder.blocks.{i}" for i in range(start, total_blocks)}

    converted = []
    # Materialize first so in-loop replacements do not alter iteration.
    named_modules = list(net.named_modules())
    for full_name, module in named_modules:
        if not isinstance(module, nn.Linear):
            continue
        if full_name.startswith("out.") or full_name == "out":
            continue
        if target_prefixes is not None:
            if not any(full_name == p or full_name.startswith(p + ".") for p in target_prefixes):
                continue
        if "." in full_name:
            parent_name, child_name = full_name.rsplit(".", 1)
            try:
                parent = net.get_submodule(parent_name)
            except Exception:
                parent = _get_submodule_fallback(net, parent_name)
        else:
            parent = net
    ...
    # Unfreeze LoRA parameters and the output head
    for name, module in net.named_modules():
        if isinstance(module, LoRALayer):
            module.lora_A.requires_grad = True
            module.lora_B.requires_grad = True
        if hasattr(net, "out") and module == net.out:
    ...
    return {
        ...
        "converted_linear_layers": len(converted),
        "converted_examples": converted[:20],
    }

def merge_and_remove_lora(net):
    for name, module in net.named_children():
        if isinstance(module, LoRALayer):
            module.merge()
            setattr(net, name, module.linear)
        else:
            merge_and_remove_lora(module)


def lora_trainability_report(net):
    """
    Summarize trainability in LoRA mode.

    Returns a dict with counts and a list of unexpectedly trainable encoder params
    (i.e., encoder params that are not LoRA A/B matrices).
    """
    total = 0
    trainable = 0
    lora_trainable = 0
    ...
    for name, p in net.named_parameters():
        ...
        if name.startswith("out."):
            out_trainable += n
        if name.startswith("encoder.") and not is_lora:
            unexpected_encoder.append(name)

    return {
        "total_params": total,
        "trainable_params": trainable,
        "lora_trainable_params": lora_trainable,
        "out_trainable_params": out_trainable,
        "unexpected_encoder_trainable": unexpected_encoder,
    }
